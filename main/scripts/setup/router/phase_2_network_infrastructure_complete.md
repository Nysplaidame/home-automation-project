...
- **VLAN 20**: **VentSys Automation** - HA, docker-host and local AI (192.168.20.0/24)
...
ping -c 3 1.1.1.1 > /tmp/connectivity_test.txt
...
# lan3:t carries VLAN 1 tagged to the managed switch, where the TL-WA801N AP port
# is configured as untagged VLAN 1/PVID 1.
# lan5:u* makes VLAN 1 the PVID on lan5 for a direct LAN/user PC or recovery laptop.
...
uci add_list network.@bridge-vlan[-1].ports='lan5:u*'  # LAN/recovery port — PVID
...
# lan2:u* sets VLAN 10 as the PVID (untagged ingress VLAN) for lan2.
...
uci set network.@bridge-vlan[-1].local='1'
uci add_list network.@bridge-vlan[-1].ports='lan1:t'  # Trunk to Proxmox for HA/docker-host/local AI
...
uci set network.@bridge-vlan[-1].local='1'
uci add_list network.@bridge-vlan[-1].ports='lan1:t'  # Trunk to Proxmox for Frigate CT
uci add_list network.@bridge-vlan[-1].ports='lan3:t'  # Managed switch trunk; camera ports are access VLAN 30
...
- VLAN 1 has lan3:t and lan5:u* assigned
...
# brctl is deprecated on DSA-based OpenWrt and not installed by default.
