...
if ping -I br-lan.1 -c 3 1.1.1.1 >/dev/null 2>&1; then
...
if ping -I br-lan.10 -c 3 1.1.1.1 >/dev/null 2>&1; then
...
if uci show firewall | grep -A5 "zone.*name='nvr'" | grep -q "network='nvr'"; then
    echo "✓ NVR zone properly mapped to network" >> /tmp/phase4_zones_test.txt
else
    echo "✗ NVR zone mapping incorrect" >> /tmp/phase4_zones_test.txt
fi

if uci show firewall | grep -A5 "zone.*name='printers'" | grep -q "network='printers'"; then
    echo "✓ Printers zone properly mapped to network" >> /tmp/phase4_zones_test.txt
else
    echo "✗ Printers zone mapping incorrect" >> /tmp/phase4_zones_test.txt
fi
...
# Verify Home Assistant to IoT Sensors access (rule name in v3.0: 'ESPHome API HA to IoT')
if uci show firewall | grep -q "ESPHome API HA to IoT"; then
    echo "✓ HA to IoT access rule configured" >> /tmp/phase4_ventsys_test.txt
else
    echo "✗ HA to IoT access rule missing (expected: 'ESPHome API HA to IoT')" >> /tmp/phase4_ventsys_test.txt
fi
...
for rule in "Bambuddy to Printer MQTT" "Bambuddy to Printer FTPS" "Docker Host AdGuard Upstream DNS" "Docker Host Tailscale Egress" "LAN to Docker Host App UIs"; do
    if uci show firewall | grep -q "$rule"; then
...
if uci show firewall | grep -q "Block NVR to Automation"; then
    ...
    echo "✗ Block NVR to Automation rule missing" >> /tmp/phase4_ventsys_test.txt
fi

# Verify printer zone rules
for rule in "Printers OTA Internet" "Block Printers Internet" "LAN to Printers"; do
    if uci show firewall | grep -q "$rule"; then
        echo "✓ Printers rule configured: $rule" >> /tmp/phase4_ventsys_test.txt
    else
        echo "✗ Printers rule missing: $rule" >> /tmp/phase4_ventsys_test.txt
    fi
done
...
if [ $guest_isolation_rules -eq 9 ]; then
    echo "✓ Guest isolation rules complete ($guest_isolation_rules/9)" >> /tmp/phase4_isolation_test.txt
else
    echo "✗ Guest isolation incomplete ($guest_isolation_rules/9)" >> /tmp/phase4_isolation_test.txt
fi
...
- **All firewall zones operational**: 12 zones configured with proper network mappings
...
- **DNS security implemented**: ordinary client zones blocked from direct external DNS; router prefers AdGuard with public fallback
