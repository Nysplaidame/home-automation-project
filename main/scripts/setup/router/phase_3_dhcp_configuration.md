...
# Configure upstream DNS servers: AdGuard first, then public fallback
uci delete dhcp.@dnsmasq[0].server
uci add_list dhcp.@dnsmasq[0].server='192.168.20.102#53'
...
# DNS for main network: local router only; router handles AdGuard/fallback
...
# DNS for management
uci add_list dhcp.@dhcp[-1].dhcp_option='6,192.168.10.1'
...
uci add_list dhcp.@dhcp[-1].dhcp_option='6,192.168.60.1'
...
# Controlled internet access; DNS through router
uci add_list dhcp.@dhcp[-1].dhcp_option='6,192.168.70.1'
...
# Guest DNS through guest router gateway; router handles upstream fallback
uci add_list dhcp.@dhcp[-1].dhcp_option='6,192.168.99.1'
...
# Docker host VM (Automation Network) — VM 103, 192.168.20.102
# First workload: Bambuddy.
uci add dhcp host
uci set dhcp.@host[-1].name='docker-host'
uci set dhcp.@host[-1].dns='1'
...
# OMV NAS (Storage Network) - PLACEHOLDER
uci add dhcp host
uci set dhcp.@host[-1].name='omv-nas'
uci set dhcp.@host[-1].dns='1'
...
uci add dhcp domain
uci set dhcp.@domain[-1].name='docker-host.home.local'
uci set dhcp.@domain[-1].ip='192.168.20.102'

uci add dhcp domain
uci set dhcp.@domain[-1].name='apt-cacher-ng.home.local'
uci set dhcp.@domain[-1].ip='192.168.20.102'
...
uci add dhcp domain
uci set dhcp.@domain[-1].name='adguard.home.local'
uci set dhcp.@domain[-1].ip='192.168.20.102'

uci add dhcp domain
uci set dhcp.@domain[-1].name='immich.home.local'
uci set dhcp.@domain[-1].ip='192.168.20.102'

uci add dhcp domain
uci set dhcp.@domain[-1].name='homepage.home.local'
uci set dhcp.@domain[-1].ip='192.168.20.102'

uci add dhcp domain
uci set dhcp.@domain[-1].name='dozzle.home.local'
uci set dhcp.@domain[-1].ip='192.168.20.102'
...
uci add dhcp domain
uci set dhcp.@domain[-1].name='omv.home.local'
uci set dhcp.@domain[-1].ip='192.168.40.50'

uci add dhcp domain
uci set dhcp.@domain[-1].name='omv-nas.home.local'
uci set dhcp.@domain[-1].ip='192.168.40.50'
...
for domain in router.home.local proxmox.home.local homeassistant.home.local docker-host.home.local bambuddy.home.local; do
