...
> Live note, 2026-05-08: VM 101 has been created from the local Debian 13
> genericcloud image instead of the older manual Debian 12 ISO path. Current
...
Use the same local cloud-image workflow as `docker-host` when possible. This is
the path already used for the live VM 101 on 2026-05-08.

```bash
ls -lh /var/lib/vz/template/iso/debian-13-genericcloud-amd64.qcow2
```

If the cloud image is missing, download it or use the older installer path
below as a fallback.
...
**OS:** preferred: Debian 13 cloud image workflow; fallback: ISO `debian-12-netinst-amd64.iso`, Guest OS: Linux 6.x
...
If you used the Debian 13 cloud image, most of this section is already handled
by cloud-init. Confirm the static IP, SSH access, hostname, and disk size, then
continue to the OS hardening and Docker steps.
...
# Bambuddy runs as a workload on docker-host VM 103. Do not open port 8000 on VM 101.
ufw enable
```
...
When the OMV NAS is online at `192.168.40.50`:
...
echo "192.168.40.50:/export/frigate /mnt/nas/frigate nfs rw,soft,intr,timeo=30,_netdev 0 0" \
    >> /etc/fstab
```
...
**VM**
- [x] VM 101 created: q35, OVMF, 4GB RAM, 64GB disk, VLAN 30
- [x] Start at boot, boot order 2
- [x] Debian 13 cloud image installed, SSH only

**OS**
- [x] Static IP: 192.168.30.20/24, gw 192.168.30.1
- [x] MAC noted and added to dhcp-config.conf
- [x] SSH key auth, password auth disabled
- [x] UFW rules applied

**Frigate**
- [x] Docker + Docker Compose installed
- [x] /opt/frigate/ structure created
...
|---|---|
| VM ID | 101 |
| OS | Debian 13 (Trixie) live path, Debian 12 (Bookworm) fallback |
