...
- [ ] Extend `Fail2ban` from the docker-host and Frigate CT baselines to the remaining Linux service hosts. Canonical jail sources now exist for the Proxmox host (`configs/proxmox/system/proxmox-fail2ban.local`, `sshd` + `proxmox` jails), monitoring VM 102 (`configs/grafana/system/monitoring-fail2ban-sshd.local`), CT 114 `llm-host` (`configs/local-ai/system/llm-host-fail2ban-sshd.local`) and OMV (`configs/omv/system/omv-fail2ban-sshd.local`), with the standard policy, pre-flight, rollback and monitoring steps in `docs/procedures/fail2ban_host_rollout.md`. HAOS, OpenWrt, the switch and cameras are excluded with written reasons. The Proxmox host (2026-07-31, `nftables-multiport`) and monitoring VM 102 (2026-08-01, `ufw`) are deployed and verified with real ban/unban proofs. CT 114 is blocked on a broken APT path. OMV is still outstanding and needs its `banaction`/backend pre-flight, a one-host window, and service-path revalidation
- [ ] Restore CT 114's package path, which is a security-update gap independent of Fail2ban: the container has no apt proxy, no outbound HTTP over IPv4 or IPv6, and docker-host's `apt-cacher-ng` ufw policy allows `3142` only from `192.168.30.20` and `192.168.60.10`. CT 114 replaced VM 104 after those rules were written. Fix is to add `192.168.20.104` to the `3142` allowances in `configs/docker-host/system/docker-host-firewall.sh` and point CT 114's APT at `http://192.168.20.102:3142`, after confirming the egress restriction is not deliberate isolation of the local-AI host
- [x] Decide the Proxmox web-UI protection path: closed on 2026-07-31 as permanently deferred. Removing the proxy's `8183` listener was trialled and reverted because it also powers Homepage's embedded Proxmox iframe preview, which is in daily use. No configuration keeps that preview and also exposes real client IPs to `pvedaemon`, so the `[proxmox]` jail stays disabled. Residual risk accepted: the internal-only Proxmox UI has no brute-force protection. Do not reopen as a gate
- [ ] Optional compensating control for the unjailed Proxmox UI: add nginx `limit_req` to the `/api2/*/access/ticket` location in the docker-host proxy's `8183` block. Rate limiting throttles credential stuffing without needing to distinguish failed from successful logins, and leaves the iframe preview intact
- [ ] Audit docker-host for other untracked live stack configs: `preview-proxy/nginx.conf` fronted eleven services and was not in the repository until 2026-07-31, so the "repo-side rebuild templates for live docker-host stacks" item was incomplete
