...
- [ ] Decide the Proxmox web-UI protection path: the `[proxmox]` jail stays disabled while the UI is reverse-proxied through docker-host `homepage-preview-proxy`, because `pvedaemon` logs all proxied failures as `192.168.20.102` and the proxy's own log cannot distinguish failed from successful logins by status code (both `200`; only body size differs). Enable it only after direct management-VLAN access to `https://192.168.10.10:8006` replaces the proxied login path; the tested filter is retained at `configs/proxmox/system/proxmox-fail2ban-filter.conf`
