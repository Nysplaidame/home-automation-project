...
- [ ] Enable the Proxmox `[proxmox]` web-UI jail. The blocker was cleared on 2026-07-31 by removing the `8183` TLS listener from the docker-host `homepage-preview-proxy`, so UI logins now go directly to `https://192.168.10.10:8006` and `pvedaemon` records real client addresses. Remaining gates: confirm a direct login works, confirm a deliberate failure logs the real `rhost` rather than `192.168.20.102`, then append the `[proxmox]` block from `configs/proxmox/system/proxmox-fail2ban.local` using the filter at `configs/proxmox/system/proxmox-fail2ban-filter.conf`
- [ ] Sign the Proxmox web-UI certificate with the local `Home Local CA` so direct `https://192.168.10.10:8006` access stops showing a browser trust warning, matching the HA and Frigate pattern
