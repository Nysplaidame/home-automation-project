...
| Secret | Bitwarden entry | Notes |
|---|---|---|
| OMV admin password | `omv-nas` | OMV web UI and SSH/admin access |
| OMV service user passwords | `omv-nas-service-users` | HA, Frigate, Immich, and backup share users |
| SMB share password | `omv-nas` | Windows `net use` and HA network storage where SMB is used |
