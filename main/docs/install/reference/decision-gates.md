...
|---|---|
| Public Internet exposure (global policy) | Default to no public exposure. Require a named owner, threat review, authenticated HTTPS, source/rate limits, secret and log policy, monitoring, patch SLA, backup/restore proof and complete route/DNS/firewall rollback before opening any inbound path. |
| Password-manager or credential store | Require trusted HTTPS, named owner/admin, 2FA and recovery-code storage, emergency-access procedure, encrypted off-host backup, isolated restore, client revocation and account recovery proof before importing real infrastructure secrets. |
...
| Low-code/agent automation (global policy) | Keep safety-critical control in HA/ESPHome/MQTT. Require a tool/action allowlist, confirmation for mutations, named credentials, bounded logs, backup/restore and disable/rollback proof before enabling flows or agents. |
...
| Camera bridge/integration (global policy) | Record camera identities, credentials, VLAN routes, discovery scope, stream ownership, iGPU/storage access and rollback to the proven Frigate/HA path before granting another bridge access. |
