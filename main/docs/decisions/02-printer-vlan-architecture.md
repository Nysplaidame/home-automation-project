---
title: Network Architecture Decision - Printer VLAN, Docker Host, VLAN 30 Rename
description: Adds VLAN 35 for printer isolation, places Bambuddy on the VLAN 20 Docker host, and renames VLAN 30 from CCTV to NVR
...
# Decision: Printer VLAN, Docker Host, and VLAN 30 Rename
...
### Decision B — Run Bambuddy on docker-host VM 103 on VLAN 20

**Chosen option:** VM 103 "docker-host" on VLAN 20 (Automation), 192.168.20.102.
Bambuddy is the first Docker Compose workload at `/opt/stacks/bambuddy`.
...
- Bambuddy is an automation integration service, not a CCTV/NVR component. It
  ...
  semantic home.
- Placing the Docker host on VLAN 20 rather than VLAN 35 (Printers) enforces a one-way
...
| `Bambuddy to Printer MQTT` | docker-host (20.102) → printers (35.200) | 8883/tcp | P1S local MQTT |
| `Bambuddy to Printer FTPS` | docker-host (20.102) → printers (35.200) | 21/tcp | P1S file transfer |
...
OpenWrt, so VLAN 20 is accepted as the trust boundary for HA+docker-host. If stricter
...
| `scripts/setup/proxmox/docker_host_setup_guide.md` | VM 103 Docker host and Bambuddy workload deployment |
| `scripts/setup/proxmox/bambuddy_vm_setup_guide.md` | Compatibility shim for old Bambuddy links |
| `README.md` | Update VLAN table, system components, hardware, key file locations |
| `PROJECT-INDEX.md` | Update VLAN table reference |
| `TO-DO.md` | Add Docker host and Bambuddy workload tasks |
| `docs/secrets_reference.md` | Update P1S IP; add Docker host section |
...
6. Create or verify VM 103 per `docker_host_setup_guide.md`
7. Remove Bambuddy from VM 101 if it exists there (`docker compose down bambuddy && docker compose up -d`)
8. Confirm HA P1S entities appear after Bambuddy is running on docker-host
...
- [[scripts/setup/proxmox/docker_host_setup_guide|Docker Host Setup Guide]] — VM 103 and Bambuddy workload deployment
