...
- Tailscale as a host-level service for daily remote access and host-route subnet routing
...
- OMV services or storage management
...
Tailscale is also allowed as a host-level service rather than a Compose workload
because it owns node identity, routes, and `tailscale0` networking for the host.
