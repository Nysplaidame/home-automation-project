...
| Local AI inference | VM 104 | `192.168.20.104` / VLAN 20 | 11434, 3002, 10200, 10300 | `llm-host.home.local`, `ollama.home.local`, `openwebui.home.local` | Not advertised by default; management/LAN/Tailscale access only if explicitly allowed | `/opt/stacks/local-ai/`; model data local to VM 104 | Uptime Kuma checks for Ollama, Open WebUI, Wyoming Piper, and Wyoming Whisper; exported to InfluxDB for Grafana service views | `scripts/setup/proxmox/llm_host_setup_guide.md`, `docs/procedures/local_ai_performance_testing.md` |
