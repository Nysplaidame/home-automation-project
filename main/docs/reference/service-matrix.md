...
| Monitoring stack | VM 102 | `192.168.60.10` / VLAN 60 | 3000, 3001, 8086 | monitoring dashboard links in HA | Not advertised | stack config + Influx backups | self-monitor plus HA-side external signal planned | `scripts/setup/proxmox/monitoring_vm_setup_guide.md` |
| Docker host | VM 103 | `192.168.20.102` / VLAN 20 | 22 plus service ports | `docker-host.home.local` | Tailscale node identity / MagicDNS | `/opt/stacks/<service>/` | Uptime Kuma, Dozzle after deploy | `scripts/setup/proxmox/docker_host_setup_guide.md` |
...
|---|---|---|---|---|---|---|---|
| Tailscale | host service | 41641/udp outbound, 443 outbound | docker-host MagicDNS | Node identity; advertises `192.168.20.101/32`, `192.168.40.50/32` | no auth keys in repo | `tailscale status`, route approval | `docs/install/phases/05-docker-host.md` |
...
| Immich | `/opt/stacks/immich/` | 2283/tcp | `immich.home.local` | Via docker-host Tailscale identity / MagicDNS | database + uploads; OMV-backed library planned | HTTP check, disk usage | `docs/install/services/immich.md` |
...
| Tier 2 | ntfy | `/opt/stacks/ntfy/` | 8085 | `docs/install/services/ntfy.md`; internal/public gate |
...
| Tier 3 | Watchtower monitor-only | `/opt/stacks/watchtower/` | none | `docs/install/services/watchtower-monitor-only.md`; monitor-only gate |
