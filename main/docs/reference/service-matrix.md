...
| Homepage | `/opt/stacks/homepage/` | 443/tcp HTTPS, 3001/tcp HTTP rollback, 8180-8208/tcp HTTPS previews | `homepage.home.local` | Via docker-host Tailscale identity / MagicDNS | config directory; live-only TLS key/cert under stack `tls/` | HTTPS check | `docs/install/services/homepage.md` |
...
| Recomp Tracker | `/opt/stacks/recomp-tracker/` | 8420/tcp | `http://192.168.20.102:8420` | Management, LAN, and docker-host Tailscale identity | persistent service data under the stack path; explicit `10.240.31.0/24` bridge | container health and HTTP `/healthz` check | `configs/docker-host/stacks/recomp-tracker/README.md` |
