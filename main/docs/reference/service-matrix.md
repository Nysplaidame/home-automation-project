...
| Frigate | CT 111 (unprivileged LXC) | `192.168.30.20` / VLAN 30 | 8971, 5000, 8554, 8555 | `frigate.home.local` | Not advertised | Frigate DB/config and non-recording media local; recordings on OMV NFS `/export/frigate` through the Proxmox-host bind mount | container/API baseline; camera monitors after hardware | `scripts/setup/proxmox/frigate_vm_setup_guide.md` |
...
| OMV NAS | NAS hardware | `192.168.40.50` / VLAN 40 storage | 80, 22, 445, 2049, NFS helper ports 111/20048/32765-32767 | `OMVNAS`; web UI at `http://192.168.40.50/`, `omv.home.local`, `nas.home.local` | not advertised as a broad storage route | md0 exports for Proxmox, HA and docker-host backups plus Immich media and root-squashed Frigate recordings; legacy direct/config aliases retired 2026-07-11 | ping, web UI, SMART, NFS checks; docker-host Telegraf sees `/mnt/omv/immich` | `scripts/setup/nas/omv_nas_setup_guide.md` |
...
| Mermaid Viewer | `/opt/stacks/mermaid-viewer/` | 8092/tcp | `mermaid-viewer.home.local` | Internal-only via docker-host Tailscale identity / LAN | static `dist/` bundle from `main/apps/mermaid-viewer/` | HTTP check | `docs/install/services/mermaid-viewer.md` |
