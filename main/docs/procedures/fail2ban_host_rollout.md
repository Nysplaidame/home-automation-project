...
## 2026-09-07 evidence reconciliation

Recovered this runbook and its five jail/filter sources from remote commit
`6bee788`; newer Homepage proxy and DHCP source were retained. This is source
recovery, not a fresh deployment or a wholesale merge of the old branch.
Authenticated read-only checks confirm VM102's `sshd` jail is active (zero
current bans); docker-host Fail2ban is active. CT114 Fail2ban is inactive, has
no configured APT proxy, cannot reach docker-host TCP3142, and direct Debian
HTTP fails. Its cached security/update indexes are dated 2026-06-19, so the
56 cached package candidates are not a current security inventory.
Proxmox and OMV reject the workstation SSH keys: the Proxmox deployment below
is historical July evidence, not September revalidation. OMV rollout remains
unproven. No bans, jail changes, package installs or firewall changes were made.

Keep the Proxmox web-UI jail deferred: the older branch documents why its
proxy sees shared client identity. Do not replace the newer preview proxy with
the July version or enable that jail while preserving the existing iframe.
