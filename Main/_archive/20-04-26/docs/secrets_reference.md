...
#   - Section 7: P1S IP updated to 192.168.35.200; Bambuddy VM reference updated
#   - Section 9: Frigate-only note added (Bambuddy removed from VM 101)
#   - Section 11: Bambuddy VM (VM 103) added
#   - Credential rotation checklist: HomePrinters and Bambuddy VM entries added
...
> Bambuddy runs on VM 103 (192.168.20.102, VLAN 20).
> See scripts/setup/proxmox/bambuddy_vm_setup_guide.md.
...
|---|---|---|
| `Bambuddy` | `ha-tokens` | Bambuddy VM → Settings → Home Assistant |
| `VentSys Dashboard` | `ha-tokens` | `HA_CONFIG.token` in ventilation_v9k.html |
...
> VM 101 runs Frigate only. Bambuddy has moved to VM 103 — see Section 11.
...
## 11 — Bambuddy VM (VM 103, 192.168.20.102, VLAN 20)
