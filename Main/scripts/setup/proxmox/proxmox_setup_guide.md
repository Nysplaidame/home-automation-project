...
> **Router status:** GL-MT6000 OpenWrt VLAN config not yet deployed.
> This guide uses a two-phase approach:
> - **Phase A–C** — configure Proxmox on your existing home network now
> - **Phase F** — cut over to the GL-MT6000 trunk once the router VLANs are live

---

## Phase A — Find Proxmox and reach the web UI

### A1 — Connect the MINIX to your existing home network

Plug the MINIX ethernet into your current home router or switch.
**Do not connect it to the GL-MT6000 yet** — the VLANs aren't live so it won't get an IP.
...
```powershell
# Adjust subnet if yours is not 192.168.1.x
...
# VLAN 10 — Management (permanent Proxmox host IP, live after router cutover)
auto vmbr0.10
iface vmbr0.10 inet static
        address 192.168.10.10/24
        gateway 192.168.10.1

# TEMPORARY — remove this block after router cutover
# Keeps web UI accessible on your home network before VLANs are live
auto vmbr0.1
iface vmbr0.1 inet static
        address 192.168.1.220/24
        gateway 192.168.1.1
        # Change .220 if that IP is taken on your home subnet
EOF
```
...
You'll briefly lose the current IP. Reconnect at `https://192.168.1.220:8006`.
Confirm `vmbr0`, `vmbr0.1`, `vmbr0.10` all appear in `pve → Network`.
...
```bash
# 1. Disable enterprise repo (requires paid licence)
...
# 2. Add free no-subscription repo
...
### Option A — Use vm-setup.sh (recommended)

Prerequisites: complete Phase D1 (HAOS image downloaded and extracted).
...
### D8 — VM 101: Download Debian 12 ISO
...
### D11 — VM 101: Note the MAC address
...
Unplug MINIX from home router. Plug into **lan1** on the GL-MT6000 (the trunk port).

### F2 — Remove the temporary IP

Access via monitor/keyboard or wait for VLAN 10 to come up, then:
...
|---|---|---|
| Proxmox (temporary) | `https://192.168.1.220:8006` | Until router cutover |
| Proxmox (permanent) | `https://192.168.10.10:8006` | After router cutover |
| Home Assistant (local) | `http://192.168.20.101:8123` | After router cutover |
...
**Proxmox host**
- [ ] Web UI reachable on temporary IP (192.168.1.220:8006)
...
- [ ] Temporary vmbr0.1 block removed after router cutover
- [ ] Proxmox reachable at 192.168.10.10:8006 after cutover
...
**VM 101 — frigate-nvr**
- [ ] VM created: q35, OVMF, VirtIO SCSI, VLAN 30, 64GB disk
- [ ] Debian 12 installed, SSH server enabled, no desktop
- [ ] ISO removed after install (`qm set 101 --delete ide2`)
- [ ] Start at boot: enabled, startup order: 2
- [ ] MAC noted and added to dhcp-config.conf (option ip 192.168.30.20)
