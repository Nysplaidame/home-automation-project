...
> **FIX #26:** The vault `configs/home-assistant/configuration.yaml` already has
> `packages: !include_dir_named packages` under `homeassistant:`. If you deployed
> the vault configuration.yaml directly, **skip this step** -- adding a second
> `homeassistant:` key silently overwrites the first, dropping all other
> settings under it (unit_system, name, etc.). Only apply the block below if
> you built configuration.yaml from scratch:

Add at the top level (not nested under anything):
...
### 3.2 — Create the packages directory
...
```bash
mkdir -p /config/www
# Then copy ventilation_v9k.html into /config/www/ventsys-dashboard.html
```

After copying, edit the file and set your Long-Lived Token:
`Settings → Profile → Security → Long-Lived Access Tokens → Create Token`
Copy the token and paste it into `HA_CONFIG.token` in the dashboard file.
...
- VentSys dashboard (`HA_CONFIG.token` in `ventilation_v9k.html`)
...
- [ ] Long-lived token created and set in dashboard file
