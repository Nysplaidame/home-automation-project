...
3. `deploy.ps1` applies only `generated/*` to the router (`network/dhcp/wireless` copied to `/etc/config`, `firewall.sh` executed).
...
Restricted client zones use `input='REJECT'` and explicit DHCP/DNS input rules.
...
- `deploy.ps1`: hardened deploy workflow with identity gate and rollback.
- `test.ps1`: post-deploy checks against UCI config (PASS/FAIL/WARN).
- `test-connectivity.ps1`: post-deploy active-state checks — per-VLAN L3 probes, dnsmasq health, live nft ruleset, zone forwarding policy.
...
`test.ps1` checks that the deployed UCI config matches expectations.
`test-connectivity.ps1` complements it by inspecting actual kernel state — VLAN
interfaces are up, dnsmasq is serving DNS, the loaded nftables ruleset matches
the firewall config, and zone-pair forwarding policy is enforced as expected.
