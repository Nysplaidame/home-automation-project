...
# lan2       10       tagged (other VLANs tagged)
# lan3       30       PVID Egress Untagged (camera POE switch)
# lan4       40       PVID Egress Untagged (NAS direct)

# Verify VLAN 1 has NO physical ports (WiFi-only)
bridge vlan show | grep -E "^lan" | grep " 1 "
# Expected: no output (VLAN 1 should not appear on any physical port)
```
...
ping -I br-lan.30 -c 3 1.1.1.1 2>/dev/null && echo "✗ VLAN 30 internet LEAKED" || echo "✓ VLAN 30 blocked"
...
```bash
for dest in 192.168.1.1 192.168.20.1 192.168.30.1 192.168.40.1 192.168.50.1 192.168.60.1 192.168.70.1; do
...
| lan3 | 30 (CCTV) | 192.168.30.100–149 |
...
# HomeMain, HomeAdmin, HomeAdmin-2G, HomeIoT, HomeGuest
...
- [ ] All 9 VLAN interfaces up with correct IPs
- [ ] VLAN 1 has no physical port assignments in `bridge vlan show`
- [ ] lan2/lan3/lan4 have correct PVIDs
- [ ] VLAN 1, 10, 99 can reach internet; 30, 40, 50 cannot
- [ ] Guest (99) cannot reach any internal VLAN
- [ ] CCTV (30) cannot reach automation (20) directly
...
- [ ] Frigate API reachable from VLAN 20 -> VLAN 30 (port 8971 for 0.14+, port 5000 for 0.13)  # A9-4 fix
- [ ] NAS NFS reachable from VLAN 30 and VLAN 20
- [ ] DHCP assigning correct ranges per VLAN
- [ ] Local DNS resolving homeassistant.home.local, frigate.home.local, nas.home.local
