...
|---|---|---|
| `Bambuddy` | `ha-tokens` | Bambuddy → Settings → Home Assistant |
...
- [ ] MQTT password — update in: Mosquitto config, HA MQTT integration, all
      ESPHome secrets.yaml files, Bambuddy MQTT settings. OTA push all boards.
...
- [ ] HA long-lived tokens — delete old in HA, create new, update
      ventilation_v9k.html and Bambuddy config
...
WireGuard keys, TLS keys, and API tokens do NOT need to be on paper --
they can all be regenerated. The six above are the minimum to rebuild the
entire system from scratch with no internet access to Bitwarden.
